...
#read in acc data 
d1 <- read.csv( "/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_full_night_and_day_data_all.csv" )
unique(d1$tag)
class(d1)
...
# 2 tags failed at some point and need to be deleted
...
for ( i in 1:length(tag_names)){
  Min <- min(d1[which(d1$tag == tag_names[i] ),]$night)
  Max <- max(d1[which(d1$tag == tag_names[i] ),]$night)
  d1 = d1[-which(d1$tag == tag_names[i] & d1$night == Min),]
  d1 = d1[-which(d1$tag == tag_names[i] & d1$night == Max),]
}

# see which are duplicated
d1[which(duplicated( d1[ , c( 'tag', 'local_timestamp' ) ] ) ),]

# remove duplicates
duplicates = d1[which(duplicated( d1[ , c( 'tag', 'local_timestamp' ) ] ) ),]
d1 = d1[-which(paste(d1$tag,d1$local_timestamp)%in%paste(duplicates$tag,duplicates$local_timestamp)==TRUE),]
...
GPSdata = GPSdata[which(GPSdata$individual.taxon.canonical.name == "Nasua narica"),]
...
percentile <- 0.49 ## this is the percentile threshold of the variable (within the noon-to-noon time period) used to classify activity vs. inactivity (VeDBA here)

waso_block <- 3 ## this is the number of consecutive minutes of inactivity needed to classify a period as sleep within the sleep period. A waso_block of 1 means that anytime the value is below the threshold, the Coati in considered sleeping and anytime the value is above the threshold the Coati is considered awake
...
## make a copy of d1. We will fill in this new dataframe with information about if the Coati was asleep in each epoch
full_dat <- d1
d1$dark = NA
...
sleep_per <- data.frame( tag = rep( unique( d1$tag ), each = length( study_nights ) ), 
                         ...
                         prev_day_sleep = NA, ## time slept during dominant waking period (here: sunrise - sunset) before the night 
                         follow_day_sleep = NA, ## time slept during dominant waking period (here: sunrise - sunset) in the following day
...
### Add in some data 
sleep_per_nona$data_col <- "FFT"
sleep_per_nona$ID <- str_split_fixed(sleep_per_nona$tag, '_', 2 )[ , 1]


### save the created dataframes as csv
write.csv( sleep_per, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_sleep_per49.csv', row.names = F )
write.csv( full_dat, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_full_dat49.csv', row.names = F )
write.csv( sleep_per_nona, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_sleep_per_nona49.csv', row.names = F )
...
round(mean(sleep_per_nona$TST_whole_day), digits = 3)
round(mean(sleep_per_nona$TST_whole_day)/60, digits = 3)

round(mean(sleep_per_nona$TST_sleep_per), digits = 3)
round(mean(sleep_per_nona$TST_sleep_per)/60, digits = 3)

round(mean(sleep_per_nona$prev_day_sleep, na.rm = TRUE), digits = 3)
round(mean(sleep_per_nona$prev_day_sleep, na.rm = TRUE)/60, digits = 3)

round(mean(sleep_per_nona$sleep_eff), digits = 3)

nrow(sleep_per_nona)

length(unique(sleep_per_nona$tag))

length(unique(sleep_per_nona$ID))
