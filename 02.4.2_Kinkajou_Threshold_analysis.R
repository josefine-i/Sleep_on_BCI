...
max(str_split_fixed( sun_trim$sunset, ' ', 2 )[ , 2 ])
min(str_split_fixed( sun_trim$sunrise, ' ', 2 )[ , 2 ])
min(str_split_fixed( sun_trim$night_end, ' ', 2 )[ , 2 ])



library(ggplot2)
ggplot(data = full_dat) + geom_density(aes(x = log_vedba, color = tag)) 
ggplot(data = full_dat[which(full_dat$tag == tag_names[11] |full_dat$tag == tag_names[13] ),]) + geom_density(aes(x = log_vedba, color = tag)) 
...
hist(d1$log_vedba, breaks = 100)

quantile(d1$log_vedba, 0.49)
...
ggplot(data = sleep_per_nona) + geom_density(aes(x = TST_sleep_per, color = tag)) 
