...
## 12.5 Hz * 3 Axes(XYZ) * seconds of sampling = number of samples
...
#check for the other frequency 
off <- acc_dat[which(acc_dat$sampling_freq_per_axis == 18.74),]
head(off)
unique(off$tag)
range(off$local_timestamp)
off$time <- str_split_fixed(off$local_timestamp, " ", 2)[,2] # check sampling across day
range(off$time)
unique(off$night) #check deployment 
length( strsplit( off$eobs_accelerations_raw[ 1 ], " " )[[1]] ) # burst length is shorter as we have the same amount of samples, but with higher freq
# choose to drop this individual 
acc_dat <- acc_dat[-which(acc_dat$sampling_freq_per_axis == 18.74),]
...
write.csv(acc_dat, "/Users/capuchin_monkey/Documents/Sleep_on_BCI/Kinkajou_sleep/DATA/Kinkajou_full_night_and_day_data.csv", row.names = F) #saves the vedba 
head( acc_dat )
