...
dy_acc <- function(vect, win_size = 7){
  ...
  #win-size can be changed, but should be default 7 for our data, in our case 7 observations are ca. 7 min (10 with pads)
...
##tag names 
#rename one Coati 
complete_data[which(complete_data$individual.local.identifier == "female Coati"),]$individual.local.identifier <- "Barbie"
...
# drop Bandida, the Zoo individual 
acc_dat <- acc_dat[-which(acc_dat$tag == "Bandida_8225"),]
unique(acc_dat$tag)
...
### downsampling sampling interval 
#current interval
difftime(acc_dat$local_timestamp[1], acc_dat$local_timestamp[2]) #should be every minute 

#round the times to the minute during which sampling occured 
acc_dat$floor_time = lubridate::floor_date(acc_dat$local_timestamp, unit = "mins")

acc_dat2 = data.frame() #output df 
for (i in tag_names){
  df = acc_dat[which(acc_dat$tag == i),]
  try = df[!duplicated(df$floor_time),]#If there are duplicate rows, only the first row is preserved
  
  acc_dat2 <- rbind(acc_dat2, try) #append to df
}

#current interval
difftime(acc_dat2$floor_time[1], acc_dat2$floor_time[2]) #should be every minute 

### length of ACC burst  
length( strsplit( acc_dat2$eobs_accelerations_raw[ 1 ], " " )[[1]] ) 
## 25 Hz * 3 Axes(XYZ) * seconds of sampling = number of samples
## 498 ACC samples collected per sample. This is 166 samples per axis. With a sampling frequency of 25 Hz per axis, this makes roughly 6.64 second bursts
## need to be: 246 ACC samples collected per minute. 82 samples per axis. Sampling frequency of 12.5 Hz per axis, makes roughly 6.56 second bursts
...
d2 <- as.data.frame( str_split( acc_dat2$eobs_accelerations_raw, " ", simplify = T ) )
...
### downsamling from 25 to 12.5 Hz (by taking every second sample)
d3 <- d2[ ,c(paste( rep(c("x","y","z"),(ncol(d2)/3)/2), # repeats x,y,z 
       rep(sequence((ncol(d2)/3)/2, from = c(1, 3), by = 2), each = 3), #string of odd numbers, so every 2nd number repeated 3x
       sep = ''))]
...
d3 <- d2[ ,c(paste( rep(c("x","y","z"),82), # repeats x,y,z 82 times
                    rep(sequence(82, from = c(1, 3), by = 2), each = 3), #every 2nd number 3x repeated for 82 numbers in total 
                    sep = ''))]

# add the timestamp and tag data to this dataframe. We can add it directly from acc_dat2 because d2 is just a column from acc_dat2 that has been split into several column (i.e. the rows still match those of acc_dat2)
d3$local_timestamp <- acc_dat2$local_timestamp
d3$tag <- acc_dat2$tag
d3$night <- acc_dat2$night
...
acc_dat2$log_vedba <- log( acc_dat2$ave_vedba ) #log that mean 

# create folder on computer to save vedba data in 
write.csv(acc_dat2, "/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_full_night_and_day_data_BCI23.csv", row.names = F) #saves the vedba 

write.csv(d3, "/Users/capuchin_monkey/Documents/Sleep_on_BCI/Coati_sleep/DATA/Coati_downsample_ACC_BCI23.csv", row.names = F) #saves downsampled ACC
head( acc_dat2 )

acc_dat2$Year <- year(acc_dat2$local_timestamp)
for (i in tag_names){
  print (i)
  print(unique(acc_dat2[which(acc_dat2$tag == i),]$Year))
}
