...
#read in acc data 
d1 <- read.csv( "/Users/capuchin_monkey/Documents/Sleep_on_BCI/Spider_sleep/DATA/Spider_full_night_and_day_data_all.csv" )
unique(d1$tag)
class(d1)
...
# see which 
d1[which(duplicated( d1[ , c( 'tag', 'local_timestamp' ) ] ) ),]

## What data do we have for data?
plot(as.numeric(as.factor(d1$tag))~d1$local_timestamp,cex=0.3,pch=16,main = "Accelerometry bursts",xlab="",xaxt='n',yaxt='n',ylab="ID")
axis(2,at=1:length(unique(d1$tag)),labels=sort(unique(d1$tag)),las=1,cex=0.3)
axis.POSIXct(1,at=seq(min(d1$local_timestamp),max(d1$local_timestamp),by="1 month"),las=2)
...
##d1=d1[-which(d1$night==unique(min(d1$night[which(as.numeric(as.character(d1$year))==2023)], na.rm = TRUE))),]
d1=d1[-which(d1$night==unique(max(d1$night[which(as.numeric(as.character(d1$year))==2023)], na.rm = TRUE))),]
...
## show how many Sider monkey nights there are
...
GPSdata = read.csv("/Users/capuchin_monkey/Documents/Sleep_on_BCI/DATA/FFT_GPS_all_cleaned.csv")
GPSdata = GPSdata[which(GPSdata$individual.taxon.canonical.name == "Ateles geoffroyi"),]
...
percentile <- 0.485 ## this is the percentile threshold of the variable (within the noon-to-noon time period) used to classify activity vs. inactivity (VeDBA here)

waso_block <- 3 ## this is the number of consecutive minutes of inactivity needed to classify a period as sleep within the sleep period. A waso_block of 1 means that anytime the value is below the threshold, the spider monkey in considered sleeping and anytime the value is above the threshold the spider monkey is considered awake
...
night_length <- 14*60  ## varaible for the minutes between 17.30 and 07.00
...
## create a vector containing the names of each spider monkey
tag_names <- unique( d1$tag )


## make a copy of d1. We will fill in this new dataframe with information about if the spider monkey was asleep in each epoch
full_dat <- d1
d1$dark = NA
...
for( i in 1:length(tag_names) ){
  ...
  for( j in 1:length(nights) ){
    ...
    if( length( time_diffs ) > 0 ){ ### There is one night for one spider monkey with only one single burst, which is why this if statement is needed
...
full_dat$eight_to_eight <- date(full_dat$local_timestamp - lubridate::hours(8))
...
d1$night = as.Date(d1$night)

study_nights <- as.Date( min( d1$night):max( d1$night ), origin = "1970-01-01", tz = "America/Panama" )


sleep_per <- data.frame( tag = rep( unique( d1$tag ), each = length( study_nights ) ), 
                         night = rep( study_nights, times = length( unique( d1$tag ))), 
                         sunset = NA, 
                         sunrise = NA, 
                         night_dur = NA, #length of the night between sunset and sunrise
                         dark_start = NA, 
                         dark_end = NA, 
                         dark_period_dur = NA, # length of night between end of astronomical twilight in the evening till start of astronomical twilight in the following morning 
                         TST_whole_day = NA, ## TST of a 24 hour period from 8 am to 8 am 
                         TST_sleep_per = NA, ## TST between 17.30 pm and 7 am 
                         WASO = NA, ## time spend awake during the period of the dominant sleep period
                         prev_day_sleep = NA, ## time slept during dominant waking period (here: 7am until 5.30pm) before the night 
                         follow_day_sleep = NA, ## time slept during dominant waking period (here: 7am - 5.30pm) in the following day
                         ...
                         night_VeDBA_corr = NA, 
                         ave_vedba = NA, ## mean VeDBA during the darkness period
                         prev_day_ave_vedba = NA, poss_dark_bursts = NA, n_dark_bursts = NA, poss_day_bursts = NA, n_day_bursts = NA, max_time_diff = NA, n_bursts= NA, max_dark_time_diff = NA, max_day_time_diff = NA)
...
for( i in 1:length(tag_names) ){
  ...
  for( j in 1:length(nights) ){
    ...
    ## copy the values needed for cleaning later 
    ...
    ## sunset and sunrise 
    sunset <- sun_dat$sunset[ which(sun_dat$date == ( as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" ) )) ]
    
    sunrise <- sun_dat$sunrise[ which(sun_dat$date == ( as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" ) )) ]
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$sunset <- sunset
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$sunrise <- sunrise
    
    night_dur <- as.numeric( sunrise - sunset, units = 'mins' )
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$night_dur <- night_dur
    ...
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$dark_start <- dark_start
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$dark_end <- dark_end
    
    dark_period_dur <- as.numeric( dark_end - dark_start, units = 'mins' )
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$dark_period_dur <- dark_period_dur
    
    
    if( nrow( night_dat[ which(night_dat$sleep_bouts == 1), ] ) > 0 ){
      
      ## TST_sleep_per = TST 17.30 to 7.00
      night_start <- as.POSIXct( paste( str_split_fixed( sunset, " ", 2 )[ , 1 ], night_start_time ), tz = 'America/Panama' )
      
      night_end <- as.POSIXct( paste( str_split_fixed( sunrise, " ", 2 )[ , 1 ], night_end_time ), tz = 'America/Panama' )
      
      SPT_dat <- night_dat[which(night_dat$local_timestamp >= night_start & night_dat$local_timestamp < night_end),]
      
      TST_sleep_per <- sum( SPT_dat$sleep_bouts == 1 )
      
      sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$TST_sleep_per <- TST_sleep_per
      ...
      sleep_eff <- TST_sleep_per/night_length
      
      sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$sleep_eff <- sleep_eff
      ...
      sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$night_VeDBA_corr <- sum( SPT_dat$log_vedba, na.rm = TRUE ) / night_length
    ...
    ## ave_vedba = mean VeDBA during the darkness period
    ...
    ## prev_day_sleep = TST during the day between 7 am and 15.30 pm
    date_start <- sun_dat$sunrise[ which(sun_dat$date == ( as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" ) - 1 )) ]
    
    day_start <- as.POSIXct( paste( str_split_fixed( date_start, " ", 2 )[ , 1 ], night_end_time ), tz = 'America/Panama' )
    
    date_end <- sun_dat$sunset[ which(sun_dat$date == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )) ]
    
    day_end <- as.POSIXct( paste( str_split_fixed( date_end, " ", 2 )[ , 1 ], night_start_time ), tz = 'America/Panama' )
    
    day_dat <- id_dat[ which(id_dat$local_timestamp > day_start & id_dat$local_timestamp < day_end), ]
    ...
    ## follow_day_sleep = time slept during dominant waking period (here: 7 am - 5.30 pm) in the following day
    follow_date <- sun_dat$sunrise[ which(sun_dat$date == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )) ]
    
    follow_day_start <- as.POSIXct( paste( str_split_fixed( follow_date, " ", 2 )[ , 1 ], night_end_time ), tz = 'America/Panama' )
    
    follow_day_end <- as.POSIXct( paste( str_split_fixed( follow_date, " ", 2 )[ , 1 ], night_start_time ), tz = 'America/Panama' )
    
    follow_day_dat <- id_dat[ which(id_dat$local_timestamp > follow_day_start & id_dat$local_timestamp < follow_day_end), ]
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$follow_day_sleep <- sum( follow_day_dat$sleep_bouts , na.rm = TRUE)
    
    ## TST_whole_day = TST of a 24 hour period from 8 am to 8 am 
    ## subset this individual's data to just that 24 h period
    twenty_four <- id_dat[ id_dat$eight_to_eight == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" ), ]
    
    TST_whole_day <- sum( twenty_four$sleep_bouts == 1 )
    
    sleep_per[ which(sleep_per$tag == tag_names[i] & sleep_per$night == as.Date( nights[j], origin = "1970-01-01", tz = "America/Panama" )), ]$TST_whole_day <- TST_whole_day
...
checkdates=data.frame(unique(cbind(d1$tag, as.character(d1$night))))
...
## Therefore, remove: sleep time during the day (day defined as prev sunrise to sunset), sleep time during the day (defined as 0730 to 1730)

sleep_per[ paste( sleep_per$tag, sleep_per$night, sep = '_' ) %in% 
             paste( sleep_per[ sleep_per$n_bursts < ( mins_in_day - missing_mins ) & !is.na( sleep_per$n_bursts ), 'tag' ], ( sleep_per[ sleep_per$n_bursts < ( mins_in_day - missing_mins ) & !is.na( sleep_per$n_bursts ) , 'night' ] + 1 ), sep = '_' ), 
           c('prev_day_sleep' ) ] <- NA
...
sleep_per[ sleep_per$n_bursts < ( mins_in_day - missing_mins ) & !is.na( sleep_per$n_bursts ), 
           c( 'TST_whole_day', 'TST_sleep_per', 'WASO', 'prev_day_sleep', 'summed_VeDBA', 'night_VeDBA_corr', 'follow_day_sleep', 'sleep_eff', 'wake_bouts', 'frag_wake_bouts') ] <- NA
...
## remove one outlier 
sleep_per_nona = sleep_per_nona[-which(sleep_per_nona$TST_whole_day > 1000),]
...
write.csv( sleep_per, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Spider_sleep/DATA/sleep_per485.csv', row.names = F )
write.csv( full_dat, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Spider_sleep/DATA/full_dat485.csv', row.names = F )
write.csv( sleep_per_nona, '/Users/capuchin_monkey/Documents/Sleep_on_BCI/Spider_sleep/DATA/sleep_per_nona485.csv', row.names = F )
...
#median
median(sleep_per_nona$TST_sleep_per)
#sd
sd(sleep_per_nona$TST_sleep_per)
#histogram to see distribution
...
#sd
sd(sleep_per_nona$sleep_eff)
#histogram to see distribution
hist(sleep_per_nona$sleep_eff, breaks = 100) 


#mean
mean(sleep_per_nona$prev_day_sleep, na.rm = TRUE)
#median
median(sleep_per_nona$prev_day_sleep, na.rm = TRUE)
#sd
sd(sleep_per_nona$prev_day_sleep,  na.rm = TRUE)
#histogram to see distribution
hist(sleep_per_nona$prev_day_sleep, breaks = 100) 
